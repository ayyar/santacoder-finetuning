...
def get_args():
    ...
    # parser.add_argument("--model_path", type=str, default="Roblox/bigcode15b")
    parser.add_argument(
    ...
    parser.add_argument("--eos_token_id", type=int, default=35262)
...
def create_datasets(tokenizer, args):
    ...
    def add_content(example):
        example[args.data_column] = example["prompt"] + example["completion"]
        return example

    dataset = dataset.map(add_content)
...
def main(args):
    tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_id, use_auth_token=True)
